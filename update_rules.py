...
def hebbian(weights, input_val, sigma, tau1, tau2, l):
    for (i, j), _ in np.ndenumerate(weights):
        weights[i, j] = weights[i, j] + input_val[i, j] * tau1 * theta(sigma[i], tau1) * theta(tau1, tau2)
        weights[i, j] = np.clip(weights[i, j], -l, l)
    return weights


def anti_hebbian(weights, input_val, sigma, tau1, tau2, l):
...
def random_walk(weights, input_val, sigma, tau1, tau2, l):
    for (i, j), _ in np.ndenumerate(weights):
