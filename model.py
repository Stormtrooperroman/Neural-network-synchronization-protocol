...
class TPM:
    def __init__(self, input_size=4, hidden_size=3, l=6):
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.l = l
        self.weights = np.random.randint(-l, l+1, [hidden_size, input_size])
        self.sigma = 0
        self.tau = 0
        self.input_val = None
    ...
    def update(self, second_tau, update_rule='hebbian'):
        ...
        if self.tau == second_tau:
            if update_rule == 'hebbian':
                self.weights = hebbian(self.weights, self.input_val, self.sigma, self.tau, second_tau, self.l)

            elif update_rule == 'anti_hebbian':
                self.weights = anti_hebbian(self.weights, self.input_val, self.sigma, self.tau, second_tau, self.l)

            elif update_rule == 'random_walk':
                self.weights = random_walk(self.weights, self.input_val, self.sigma, self.tau, second_tau, self.l)
            else:
